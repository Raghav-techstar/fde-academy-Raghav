...
@patch("day5_ex2_logistics_client.mock_http_get")
def test_403_forbidden_raises_api_error(mock_get, client):
    """403 should raise APIClientError immediately without retry."""
    ...
    with pytest.raises(APIClientError):
        client.get_shipment("SH-401")

    assert mock_get.call_count == 1


@patch("day5_ex2_logistics_client.mock_http_get")
def test_unexpected_status_raises_api_error(mock_get, client):
    """Unexpected status codes should raise APIClientError."""

    mock_get.return_value = (
        418,
        json.dumps({"error": "I'm a teapot"}),
    )

    with pytest.raises(APIClientError):
        client.get_shipment("SH-402")
...
@patch("day5_ex2_logistics_client.time.sleep")
@patch("day5_ex2_logistics_client.mock_http_get")
def test_retries_exhausted(mock_get, mock_sleep, client):
    """Should retry four times then raise APIClientError."""
...
from day5_ex2_logistics_client import _call_log


def test_mock_http_get_real_sequence():
    """Exercise the real mock_http_get implementation."""
    ...
    result = client.get_shipment("SH-777")

    assert result["shipment_id"] == "SH-777"
