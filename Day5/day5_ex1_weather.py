...
MOCK_API_RESPONSES: dict[str, dict] = {
    ...
    "Chennai": {
        ...
        "current": {
            ...
            "wind_kph": 9.0,
            "condition": {
                "text": "Humid and partly cloudy",
            },
            # air_quality intentionally missing
        },
        "forecast": {
            "forecastday": [
                {
    ...
    "Bengaluru": {
        ...
        "forecast": {"forecastday": []},  # Empty forecast
    },
    "Delhi": {
        "location": {
            "name": "Delhi",
            "country": "India",  # region intentionally missing
        },
        "current": {
            "temp_c": 18.0,
            "humidity": 40,
...
def mock_get_weather(city: str) -> dict:
    """
    Simulate an HTTP GET call to a weather API.
...
def transform_weather_to_dashboard(raw: dict) -> DashboardWeatherRecord:
    ...
    return DashboardWeatherRecord(
        ...
        pm2_5=air.get("pm2_5"),
        tomorrow_max_c=tomorrow.get("day", {}).get("maxtemp_c"),
        tomorrow_min_c=tomorrow.get("day", {}).get("mintemp_c"),
        tomorrow_outlook=tomorrow.get("day", {}).get("condition", {}).get("text"),
    )
...
def build_dashboard_panel(cities: list[str]) -> list[DashboardWeatherRecord]:
    ...
    for city in cities:
        try:
            raw_weather = mock_get_weather(city)
            dashboard_record = transform_weather_to_dashboard(raw_weather)
            panel.append(dashboard_record)
...
if __name__ == "__main__":
    ...
    for record in panel:
        print(
            f"{record.city:<20} {record.temp_c:>5.1f}C {record.condition:<25} "
