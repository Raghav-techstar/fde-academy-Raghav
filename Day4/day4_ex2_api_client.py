...
class CarrierAPIClient:
    ...
    def _handle_status(
        ...
        5xx -> raise APIClientError (server error)
        4xx -> raise APIClientError (client error)
        other -> raise APIClientError
        """
    ...
    # ■■ PROVIDED: HTTP simulator — do not modify ■■■■■■■■■■■■■■■■■■■■■■■■■■
...
if __name__ == "__main__":
    ...
    with CarrierAPIClient(
        ...
        print(f"\nRetrieved {len(shipments)} shipments:\n")

        for shipment in shipments:
            print(shipment)

    # ----------------------------------------------------
    # Test RuntimeError (calling outside context manager)
    # ----------------------------------------------------
    try:
        client.get_shipments("2024-01-20")
    except RuntimeError as e:
        print(f"\nExpected RuntimeError: {e}")

    # ----------------------------------------------------
    # Test DataParseError
    # ----------------------------------------------------
    try:
        # Open the session manually for testing
        client._session_open = True

        client._parse_response("http://test", "not valid json {{")

    except DataParseError as e:
        print(f"\nExpected DataParseError: {e}")
