...
class ShipmentOrder(BaseModel):
    ...
    def check_route(self) -> "ShipmentOrder":
        if self.origin.lower() == self.destination.lower():
            raise ValueError("check_route: origin and destination must differ")
        return self
...
class ProcessedEquipment(Asset):
    ...
    def estimated_value(self) -> float:
        return round(
            self.order.purchase_price
            * max(0, 1 - self.order.age_years / self.DEPRECIATION_YEARS),
            2,
        )
...
class AssetFactory:
    ...
    def create(order: ShipmentOrder | EquipmentOrder) -> Asset:
        ...
        elif isinstance(order, EquipmentOrder):
    ...
    def run(self, raw_orders: list[dict]) -> ProcessingResult:
        ...
        for raw in raw_orders:
            ...
            try:
                ...
                for err in e.errors():
                    msg = err["msg"]
                    # Remove redundant "Value error, " prefix
                    if msg.startswith("Value error, "):
                        msg = msg.replace("Value error, ", "", 1)
                    # Add field name prefix if available (for field validators)
                    if err["loc"]:
                        msg = f"{err['loc'][0]}: {msg}"
                    errors.append(msg)
                ...
                log.warning("Validation failed for %s: %s", order_id, [str(e)])
